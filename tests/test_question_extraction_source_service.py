...
from app.services.question_extraction_source_service import (
    QuestionExtractionSourceMetadataError,
    QuestionExtractionSourceMetadataNotFoundError,
    QuestionExtractionSourceResolutionError,
    QuestionExtractionSourceService,
    QuestionExtractionSourceValidationError,
...
class QuestionExtractionSourceServiceTest(unittest.TestCase):
    def setUp(self) -> None:
        self.db = MagicMock()
        ...
        self.media = SimpleNamespace(
            id=self.media_id,
            storage_key="sources/2026/08/book.pdf",
    ...
    def _service(self) -> QuestionExtractionSourceService:
        return QuestionExtractionSourceService(
            self.db,
    ...
    def test_resolution_projects_exact_safe_metadata_and_readable_stream(
        self,
    ) -> None:
    ...
    def test_run_id_is_strict_uuid_before_database_or_storage_access(
        self,
    ) -> None:
        for run_id in ("bad", 1, True, None):
            with self.subTest(run_id=run_id), self.assertRaises(
                ...
                with self._service().open_for_run(
    ...
    def test_inconsistent_relationships_are_rejected_before_storage(
        self,
    ) -> None:
        cases = (
        ...
        for run_document_id, document_media_id in cases:
            with self.subTest(
                ids=(run_document_id, document_media_id)
            ):
    ...
    def test_storage_failure_is_translated_without_sensitive_details(
        self,
    ) -> None:
    ...
    def test_database_failure_is_typed_without_sensitive_details(
        self,
    ) -> None:
    ...
    def test_service_is_read_only_and_has_no_lifecycle_or_processor_boundary(
        self,
    ) -> None:
        self.storage.open_key.return_value = TrackingStream(b"source")
        ...
        module = Path(
            BACKEND_DIR
            / "app/services/question_extraction_source_service.py"
